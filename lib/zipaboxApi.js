...
class ZipaboxApi{
  constructor(debug,url,log,user,password){
    ...
    if(this.debug)
      this.log.warn("Debug mode activated for API request.")
  ...
  connectUser(){
    return new Promise(function(resolve, reject){
      ...
      if(this.inConnection == false){
        this.debug && this.log("Connection launched.")
        this.inConnection = true;
        resolve(this.initUser().then(this.loginUser.bind(this)));
      }else{
        this.debug && this.log("Reconnection already request.")
  ...
  initUser(){
    return new Promise(function(resolve, reject) {
      ...
      .catch(function manageError(error) {
        console.log('Error occurred in initUser :', error);// TODO ADD gestion Error
        reject(error);
      });// end fetch chaining
  ...
  */
  loginUser(nonce){
    return new Promise(function(resolve,reject){
      this.debug && this.log(">>> Method loginUser()");
      ...
      .then(function giveResult(jsonReponse){
        this.debug && this.log("jsonresponse for login : ",jsonReponse);
        this.inConnection = false;
  ...
  */
  logoutUser(){
    return new Promise(function (resolve, reject){
      this.debug && this.log(">>> Method logoutUser()");
      /* Logout the user */
      fetch(this.baseURL +'user/logout',myInitGet)
      .then(fstatus)
      .then(fjson)
      .then(function giveResult(jsonReponse){
        this.debug && this.log("jsonresponse for logout : ",jsonReponse);
        this.inConnection = false;
        resolve(jsonReponse.success);
      }.bind(this))
      .catch(function manageError(error) {
  ...
  */
  getDeviceUUID(attributeUUID){
    return new Promise(function(resolve, reject){
      ...
      this.debug && this.log(">>> Method getDeviceUUID()");
  ...
  */
  connectFirstSecurity(pin){
    /* DEBUG */
    this.debug && this.log(">>> Method connectFirstSecurity()");
  ...
  */
  connectSecurity(){
    /* DEBUG */
    this.debug && this.log(">>> Method connectSecurity()");
  ...
  */
  initSecurity(){
    return new Promise(function(resolve, reject) {
      ...
      .catch(function manageError(error) {
        console.log('Error occurred in initSecurity :', error);// TODO ADD gestion Error
        reject(error);
      });// end fetch chaining
    }.bind(this));// End Promise
  } // end initSecurity
  ...
  */
  loginSecurity([secureSessionId, nonce, salt, pin]){
    ...
    return new Promise(function(resolve,reject){
      this.debug && this.log(">>> Method loginSecurity()");
      //this.debug && this.log("secureSessionId for loginSecurity :",secureSessionId);
      //this.debug && this.log("Nonce for loginSecurity :",nonce);
      //this.debug && this.log("Salt for loginSecurity :",salt);
      /* Calculate saltPin */
      if(pin == "noPin"){
        this.log.error("No Pin specified - Connection to security not possible.");
      ...
      this.debug && this.log("URL pour loginSecurity: " + this.baseURL +'security/session/login/'+secureSessionId+'?token='+token);
      ...
      .then(fjson)
      .then(function giveResult(jsonReponse){
        //this.debug && this.log("Result loginSecurity",jsonReponse);
        this.debug && this.log("Request to the loginSecurity : ",jsonReponse.success);
        if(jsonReponse.response.success == false)
          this.log.error("Connection to the loginSecurity : ",jsonReponse.response.success); // TODO manage error if false
        else
          this.debug && this.log("Connection to the loginSecurity : ",jsonReponse.response.success);
        resolve(jsonReponse.success);
      }.bind(this))
      .catch(function manageError(error) {
        reject(error);
  ...
  */
  getDeviceStatus(uuidDevice,noStatus){ // Return the device Status
    ...
    //this.debug && this.log(">>>> Parameters : ", uuidDevice, noStatus);
    if(noStatus){ //config say that no device is available > return true
      ...
      return new Promise(function(resolve, reject) {
        ...
        .catch(function manageError(error) {
          //console.log('Error occurred in getDeviceStatus :', error);// TODO ADD gestion Error
          reject(error);
        });// end fetch chaining
      }.bind(this));// End Promise
    }
  ...
  */
  getDeviceBatteryLevel(uuidDevice){ // Return the device Battery Level
    return new Promise(function(resolve, reject) {
      ...
      .catch(function manageError(error) {
        console.log('Error occurred on getDeviceBatteryLevel :', error);// TODO ADD gestion Error
        reject(error);
      });// end fetch chaining
    }.bind(this));// End Promise
  ...
  */
  getAttributesValue(uuidAttributes){ // Just method to maintain the request, not the value (need to be done)
    return new Promise(function(resolve, reject){
      ...
      .catch(function manageError(error) {
        console.log('Error occurred on getAttributesValue  :', error);// TODO ADD gestion Error
        reject(error);
      });// end fetch chaining
    }.bind(this));// End Promise
    ...
    return new Promise(function(resolve,reject){
      ...
      .then(function giveResponse(jsonResponse){
        ...
        if (jsonResponse.success == false){
          this.log.error("Post method error : success false is returned");
          ...
          if(jsonResponse.success == true && jsonResponse.zoneStatuses[0] === undefined){
            ...
            */
            if(jsonResponse.zoneStatuses[0].ok == false){
              ...
              this.log.error("ERROR : bad return of Post Security Method");
      ...
      .catch(function manageError(error) {
        this.log.error('Error occurred!', error);// TODO ADD gestion Error
        reject(error);
      }.bind(this));// end fetch chaining
    }.bind(this)); // end Promise
  ...
  */
  putAttributesValueRequest(uuid,valueBool){
    return new Promise(function(resolve, reject){
      ...
      .catch(function manageError(error) {
        this.log.error('Error occurred on putAttributesValueRequest :', error);// TODO ADD gestion Error
        reject(error);
      }.bind(this));// end fetch chaining
    }.bind(this));// End Promise
...
function fstatus(response){
  return new Promise(function(resolve,reject){
    ...
    if (response.status >= 200 && response.status < 300) {
      ...
      //console.error("Error on fstatus.", response.statusText);
