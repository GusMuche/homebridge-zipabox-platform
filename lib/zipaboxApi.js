      ...
      return new Promise(function(resolve, reject) {
        ...
        .catch(function manageError(error) {
          if(error.message == "No JSON" || error.type == 'invalid-json'){
            this.debug && this.log("[BOXAPI] [getDeviceStatus] Invalid JSON manage to force true. Accessory fix as status ok");
            resolve(true);
          }else{
  ...
  */
  isOnSector(uuidDevice){
    return new Promise(function(resolve,reject){
      ...
      .catch(function manageError(error) {
        if(error.message == "No JSON" || error.type == 'invalid-json'){
          this.debug && this.log("[BOXAPI] [isOnSector] Invalid JSON manage to force true. Accessory fix as on sector.");
          resolve(true);
        }else{
