...
class ZipAccessory{
  ...
  */
  fixContextFromConfig(config){
    ...
    if(this.timePolling != 0)
      this.debug && this.log("[ZIPAC] [fixContextFromConfig] User request to refresh the value after (seconds)",this.timePolling);
  ...
  */
  statusPolling(){
    /* DEBUG */
    this.debug && this.log("[ZIPAC] > Method statusPolling");
  ...
  getStatusBattery (callback){
    ...
    .then(function manageStatus(deviceStatus){
      return new Promise(function(resolve,reject){
        ...
        this.debug && this.log("[ZIPAC] [getStatusBattery] Type of Test Value in manage Status : ",typeof(deviceStatus));
    ...
    .then(function (batteryValue){
      ...
      if(batteryValue < this.batteryLimit)
        underLevel = 1;
      this.debug && this.log("[ZIPAC] [getStatusBattery] Battery status returned to callback:",underLevel);
      callback(error,underLevel);
    }.bind(this))
    .catch(function manageError(error){
      //this.log("Test Value in manage Error : ",deviceStatus);
      this.log("[ZIPAC] [getStatusBattery] Error on getOn :",error);
  ...
  */
  getOn(callback){
    /* DEBUG */
    this.debug && this.log("[ZIPAC] > Method getOn for ", this.name);
    this.debug && this.log("[ZIPAC] [getOn] device UUID and nostatus :",this.deviceUUID,this.noStatus,this.isOnG);+

    var error = null;
    /* Check for unhadled error */
    if(this.deviceUUID == undefined || this.deviceUUID == null){
      error = "[ZIPAC] [getOn] this is forgot. Unhadled error > try reset cachedAccessories.";
    ...
    this.zipabox.getDeviceStatus(this.deviceUUID,this.noStatus)
    .then(function checKIfAlreadyOngoing(){
      return new Promise(function(resolve,reject){
    ...
    .then(function (accessoryValue){
      ...
      if(this.reverseValue == true){ // User ask to reverse
        if(typeof(returnedValue) != "boolean"){ // Check if returnedValue is a Boolean
          var error = new Error("[ZIPAC] [getOn] Coding error in getOn: returnedValue is not a boolean in reverseValue");
          this.log(error);
          throw error;
        }else{
          if(returnedValue == true)
        ...
        this.debug && this.log("[ZIPAC] [getOn] Configuration have request to reverse the value to :",returnedValue)
      } // end reverse block
      ...
      if(this.type == "battery"){
        if(accessoryValue == undefined)
          this.log.error("[ZIPAC] [getOn] Returned value for the battery level is undefined !"); // TODO add error manage
        else
    ...
    .catch(function manageError(error){
      ...
      if(error == "getOnOngoing"){
        this.debug && this.log.warn("[ZIPAC] [getOn] New get Request before get the previously one. By-passed. Reduce the refresh rate to avoir this.");
        this.getOnOngoing = false;
        callback(null,this.lastValue);
      }else{
        //this.log("Test Value in manage Error : ",deviceStatus);
        this.log.error("[ZIPAC] [getOn] Error on getOn :",error);
        this.getOnOngoing = false;
        callback(error,undefined);
         //throw new Error(error);
      }
  ...
  */
  getOnB (callback) {
    /* DEBUG */
    this.debug && this.log('[ZIPAC] > Method getOnB');

    var error = null;
    this.zipabox.getAttributesValue(this.uuidb)
    .then(function (accessoryValue){
       this.debug && this.log("[ZIPAC] [getOnB] Accessory Value returned by callback B:",accessoryValue);
       var returnedValue = accessoryValue;
       if(this.type == "battery"){
         this.debug && this.log("[ZIPAC] [getOnB] Battery to manage in getOnB. returnedValue :",returnedValue)
    ...
    .catch(function manageError(error){
     ...
     this.log.error("[ZIPAC] [getOnB] Error on getOnB :",error);
     callback(error,undefined);
        //throw new Error(error);
  ...
  */
  getOnLast (callback){
    /* DEBUG */
    this.debug && this.log("[ZIPAC] > Method getOnLast");
  ...
  */
  setOn(value, callback){
    /* DEBUG */
    this.debug && this.log("[ZIPAC] > Method setOn", value);
    /* Refresh state for some special accessories */
    if(this.type == "window"){
      this.debug && this.log("[ZIPAC] [setOn] Set method for a Window NOT IMPLEMENTED > stop signal");
      this.platformAccessory.getService(Service.Window).getCharacteristic(Characteristic.CurrentPosition).getValue();
      this.platformAccessory.getService(Service.Window).getCharacteristic(Characteristic.TargetPosition).getValue();
      callback(null);
      return;
    ...
    if(this.type == "door"){
      this.debug && this.log("[ZIPAC] [setOn] Set method for a door NOT IMPLEMENTED > stop signal");
      this.platformAccessory.getService(Service.Door).getCharacteristic(Characteristic.CurrentPosition).getValue();
      this.platformAccessory.getService(Service.Door).getCharacteristic(Characteristic.TargetPosition).getValue();
      callback(null);
      return;
  ...
  */
  getOnSecurity (callback) { // Use for get Alarm status
       /* Log to the console the value whenever this function is called */
       this.debug && this.log('[ZIPAC] > Method getOnSecurity');
       ...
       .catch(function manageError(error){
         this.log("[ZIPAC] [getOnSecurity] Error on getOnSecurity :",error);
         callback(error,undefined);
          //throw new Error(error);
       }.bind(this));
  } // end getOnSecurity
  ...
  */
  setOnSecurity (value, callback){ // set method for alarm Type (SecuritySystem)
    ...
    .then(function checkIfTrue(putBooleanResponse){
      this.debug && this.log("[ZIPAC] [setOn] return of putBooleanResponse :",putBooleanResponse);
      this.debug && this.log("[ZIPAC] [setOn] type of putBooleanResponse :",typeof(putBooleanResponse));
      if(putBooleanResponse == false){ // The Box have return an issue in arming the Security System
        ...
        if(value == 0 && this.nightMode){ // Set on Stay (=Home), but nightMode configured
          this.log.warn("[ZIPAC] [setOn] User set alarm on STAY / HOME but nightMode is on true. Force to NIGHT")
          this.platformAccessory.getService(Service.SecuritySystem).getCharacteristic(Characteristic.SecuritySystemTargetState).getValue();
        }
        if(value == 2 && !this.nightMode){ // Set on Night, but nightMode configured
          this.log.warn("[ZIPAC] [setOn] User set alarm on NIGHT but nightMode is on false. Force to HOME.")
          this.platformAccessory.getService(Service.SecuritySystem).getCharacteristic(Characteristic.SecuritySystemTargetState).getValue();
        }
        callback(null);
      }
    ...
    .catch(function manageError(error) {
      throw new Error("[ZIPAC] [setOn] Undefined error :", error);
    });
  ...
  reconnectAfterError(error){
    ...
    return new Promise(function(resolve, reject){
      if (error.message == "Unauthorized" || error.message == "Unauthorized "){ // || error.message == "Bad Request " > for test
        ...
        this.debug && this.log("[ZIPAC] [reconnectAfterError] Found getOnOngoing error. Return last value.")
