...
class ZipAccessory{
  ...
  getOn(callback){
    ...
    if(needApiResquest == false && this.useCache == true){ // useCache without reconnection needed
      ...
      .then(function sendCallBack(returnOfApi){
        this.getOnOngoing = false;
        this.debug && this.log("[ZIPAC] [getOn] Callback the new value saved in last value", this.debugContext);
        callback(error, this.lastValue);
      }.bind(this))
      .catch(function manageError(error){
  ...
  */
  reconnectAfterError(error){
    ...
    return new Promise(function(resolve, reject){
      if (error.message == "Unauthorized" || error.message == "Unauthorized " || error.message.substr(0,38) == "request to https://my3.zipato.com:443/"){ // || error.message == "Bad Request " > for test
