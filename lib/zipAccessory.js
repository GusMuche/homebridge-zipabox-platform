...
class ZipAccessory{
  constructor(debug,log,zipabox,zAccessory,zService,zCharacteristic,accessoryJSON,uuidGen){
    ...
    this.deviceBatteryLevel = 100;
    this.deviceTrouble = false;
    this.deviceIsOnSector = false; // "mainsPower"
    this.special = false;
    ...
    /* Battery option */
    this.batteryBound = false; // true if at least one service have the battery limit active
    // this.batteryLevel = 100; // replaced by this.deviceBatteryLevel
    ...
    switch(type){
      ...
      case "window":
        this.platformAccessory.getService(serviceName) // Service.Window)
          ...
          .on('get', function(callback){this.getCharacteristicValue(index, callback);}.bind(this));
          //.on('set', this.setOn.bind(this)); // No set for this method implemented
        this.platformAccessory.getService(serviceName) // Service.Window)
          .getCharacteristic(Characteristic.TargetPosition)
          .on('get', function(callback){this.getOnLast(index, callback);}.bind(this))
          .on('set', function(value, callback){this.setOn(index, value, callback);}.bind(this)); // in fact to nothing
        this.platformAccessory.getService(serviceName) // Service.Window)
          .getCharacteristic(Characteristic.PositionState)
          .on('get', function(callback){callback(null,2);}) // Always return 2 = stopped
          .on('set', function(callback){callback(null);}); // Always return ok
        this.platformAccessory.getService(serviceName) // Service.Window)
      ...
      case "co2":
        this.platformAccessory.getService(serviceName) // Service.CarbonMonoxideSensor
          ...
          .on('get', function(callback){this.getCharacteristicValue(index, callback);}.bind(this));
        break;
      case "battery": // AT HERE TO TEST
        this.platformAccessory.getService(serviceName) // Service.BatteryService
          .getCharacteristic(Characteristic.BatteryLevel)
          .on('get', function(callback){this.getCharacteristicValue(index, callback);}.bind(this));
        this.platformAccessory.getService(serviceName) // Service.BatteryService
          .getCharacteristic(Characteristic.ChargingState)
          .on('get', function(callback){this.getCharacteristicValueB(index, callback);}.bind(this));
        break;
      case "alarm":
  ...
  */
  identifyZipAccessory(){
    ...
    this.log.warn(this.debugContext,"isOnSector ? :",this.deviceIsOnSector);
  ...
  */
  bindBatteryLowStatus(type){
    ...
    this.services[0].getCharacteristic(Characteristic.StatusLowBattery) // Normal = 0, Low = 1
      .on('get', this.getStatusBattery.bind(this));

    /* Bound the battery percentage function */
    if(this.useEve){
      if(this.services[0].testCharacteristic(ZHBatteryPercentage)){ // Only need an update
        this.services[0].getCharacteristic(ZHBatteryPercentage)
          .on('get', this.getBatteryLevel.bind(this));
      }else{
        this.services[0].addCharacteristic(ZHBatteryPercentage); // Need to add
        this.services[0].getCharacteristic(ZHBatteryPercentage)
          .on('get', this.getBatteryLevel.bind(this));
      }
    }
  ...
  /*
  Function to request and save the battery level
  */
  updateBatteryLevel(newLevel){
    /* DEBUG */
    this.debug && this.log("[ZIPAC] > Method updateBatteryLevel ", newLevel, this.debugContext);

    /* avoid one update without battery available */
    if(this.batteryBound){
       /* Battery is available */
      var statusBattery = 0;
      if(this.batteryLimit > newLevel)
        statusBattery = 1; // battery is low

      this.services[0].updateCharacteristic(Characteristic.StatusLowBattery,statusBattery);
    }
  }
  ...
  directUpdate(index,value){
    ...
    switch(type){
      ...
      case "contact":
        ...
        break;
      case "window":
        service.updateCharacteristic(Characteristic.CurrentPosition, value);
        service.updateCharacteristic(Characteristic.TargetPosition, value);
        //service.updateCharacteristic(Characteristic.PositionState,2); // NEED?
        //service.updateCharacteristic(Characteristic.HoldPosition,false); // NEED ?
        break;
      case "door":
        service.updateCharacteristic(Characteristic.CurrentPosition, value);
        service.updateCharacteristic(Characteristic.TargetPosition, value);
      ...
      case "co2":
        ...
        break;
      case "battery":
        service.updateCharacteristic(Characteristic.BatteryLevel).getValue();
        service.updateCharacteristic(Characteristic.ChargingState).getValue();
        break;
      // case "alarm":
      //   service.getCharacteristic(Characteristic.SecuritySystemCurrentState).getValue();
      //   service.getCharacteristic(Characteristic.SecuritySystemTargetState).getValue();
      ...
      //   break;
      default:
        this.log.error("[ZIPAC] [directUpdate] No type found for", type);
    } // end switch
  ...
  */
  getType(attributeUUID){
    ...
    // }
    var indexOfService = this.getServiceIndex(attributeUUID);
    typeToReturn = this.types[indexOfService];
    return typeToReturn;
  }

  /*
  Return the index from 0 to max of the services corresponding to attribute UUID
  */
  getServiceIndex(attributeUUID){
    var indexToReturn = -1;
    for (var i in this.uuids){
      if(this.uuids[i] == attributeUUID)
        indexToReturn = i;
    ...
    if(indexToReturn == -1)
      this.log.error("[ZIPAC] [getServiceIndex] Bad index asked : not found for UUID ",attributeUUID);
  ...
  */
  setDeviceStatusFromPlatform(online,batteryLevel,trouble,mainsPower){
    // if(online != true && online != false){ // TODO TO DELETE AFTER TEST
    //   throw new Error("Incorrect state of status to set. Must be boolean");
    // }else{
      this.deviceOnline = online;
      if(this.deviceBatteryLevel == batteryLevel){
        // do nothing > same level
      }else{
        this.deviceBatteryLevel = batteryLevel;
        this.updateBatteryLevel(batteryLevel); // mise à jour niveau
      }
      this.deviceTrouble = trouble;
      this.deviceIsOnSector = mainsPower
    //}
    //}
  }
  ...
  getDeviceUUID(){
    ...
    return new Promise(function (resolve,reject){
      ...
      /* If not e check if it's allow to search one */
        ...
        this.deviceIsOnSector = true;
        resolve(this.deviceUUID);
      }else{ // Device UUID not knowed and noStatus is false
        ...
        .then(function setIfIsOnSector(deviceUUID){
          ...
          .then(function checkValueSector(isOnSector){
            ...
            if(typeof(isOnSector) == "boolean"){
              this.deviceIsOnSector = isOnSector;
              this.noStatus = isOnSector;
            }
  ...
  */
  getStatusBattery(callback){
    ...
    var statusBattery = 0;
    if(this.batteryLimit > this.deviceBatteryLevel)
      statusBattery = 1; // battery is low
  ...
  //     // return this.reconnectAfterError(error); // NO MORE Implemented > to delete after improvement
  ...
  */
  getBatteryLevel(callback){
    ...
    callback(null,this.deviceBatteryLevel);
  } // end method getBatteryLevel

  /*
  Return the value /lastValues(index)/ for the specific characteristic, specified by the index
  */
  getCharacteristicValue(index, callback){
    /* DEBUG */
    this.debug && this.log("[ZIPAC] > Method getCharacteristicValue", index, this.debugContext);
    this.debug && this.log("[ZIPAC] [getCharacteristicValue] deviceStatus", this.deviceOnline, this.debugContext);
    this.debug && this.log("[ZIPAC] [getCharacteristicValue] noStatus", this.noStatus, this.debugContext);

    var error = null;
    if(this.deviceOnline == false && this.noStatus == false && this.deviceIsOnSector == false){
    ...
    if(this.firstGet){ // if plugin launch first time, we need to wait a little bit to use the lastValues
      ...
      this.debug && this.log("[ZIPAC] [getCharacteristicValue] lastValue returned", this.lastValues[index], this.debugContext);
  ...
  */
  getEmptyValue(index){
    ...
    this.debug && this.log("[ZIPAC] [getEmptyValue] Value used here :", emptyValueToReturn, "for index", index, this.debugContext);
  ...
  */
  translateAttribute(attribute,uuid){
    ...
    var indexOfService = this.getServiceIndex(uuid);
    ...
    if(type == "ambient"){ // returned from % to scale
      returnedValue = Math.round(this.mins[indexOfService] + returnedValue/100 * this.ranges[indexOfService]);
    } // end if ambient
    ...
    this.debug && this.log("[ZIPAC] [translateAttribute] Value returned:", returnedValue, this.debugContext)
  ...
  */
  ...
  /*
  getOnLast give the lastvalue
  This is used for different Characteristic who need same request to the box
  Avoid multiple request to the box
  Delay of 2sec to have the get
  */
  getOnLast (index, callback){
    /* DEBUG */
    this.debug && this.log("[ZIPAC] > Method getOnLast", this.debugContextServices[index]);

    this.delay(2000,this.lastValues[index])
    .then(function sendTheCallback() { // here value is the lastValue
      var error = null;
      if(this.lastValues[index] == undefined){
        error = "[ZIPAC] [getOnLast] Method request without a value set. Retry later."
        this.log.warn(error, this.debugContextServices[index]);
      }
      callback(error, this.lastValues[index]);
    }.bind(this));
  } // end getOnLast
  ...
  */
  ...
  /*
  setAlarm is used to maybe update the value of the status of an alarm
  If same no action
  If new, save it and update homebridge
  */
  setAlarm(newState){
    /* DEBUG */
    this.debug && this.log("[ZIPAC] > Method setAlarm :", newState, this.debugContext);

    // TODO WORK WITH ALARM IN ARRAY !
    
    // if(newState >= 10){ // TAMPERED
    //   this.isTampered = 1;
  ...
  */
  setOn(index, value, callback){
    ...
    if(this.types[index] == "window"){
      ...
      this.directGet(index); // TODO TEST TO BE DONE
    ...
    .then(function launchCallBack(resp){
      ...
      if(this.lastValues[index] != 0 && this.types[index] == "dimmer") // see setOnRange
        this.backupValues[index] = valueToPut;
      callback(resp);
    }.bind(this))
    .catch(function manageError(error) {
      // throw new Error(error);
      this.log.error("Unmanaged error during setOn(). Contact developper ", this.debugContextServices[index]);
      callback(error);
    }.bind(this));
  } // end setOn function
  ...
  Just tested on dimmer accessory
  */
