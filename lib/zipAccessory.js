...
class ZipAccessory{
  constructor(debug,log,zipabox,zAccessory,zService,zCharacteristic){
    ...
    this.refreshCount = 0; // Use to know how many times we call for the battery status
  ...
  */
  getOn(callback){
    ...
    if(this.refreshCount >= 10){
      this.debug && this.log("[ZIPAC] [getOn] RefreshCount =",this.refreshCount);
      this.refreshCount = 0;
      if(!this.isOnSector){
        if(this.basedService.getCharacteristic(Characteristic.StatusLowBattery)){ // We already have it bound
