...
class ZipAccessory{
  constructor(debug,log,zipabox,zAccessory,zService,zCharacteristic,accessoryJSON,uuidGen){
    ...
    this.first = true; // true for first launch, false after
  ...
  */
  getCharacteristicValue(index, callback){
    ...
    if(this.first && this.nbServices == index){ // didnt check the lastValue for error the first time
      this.first = false;
    }else{
      if(!this.first && (this.lastValues[index] == null || this.lastValues[index] == undefined || index > this.nbServices )){
