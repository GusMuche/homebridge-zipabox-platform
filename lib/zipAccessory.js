...
class ZipAccessory{
  ...
  */
  fixContextFromConfig(config, fromCache = false){
    ...
    if(this.noStatus) // If no status, we not search a battery
    ...
    if(this.type == "door" || this.type == "window" || this.noStatus)
      this.batteryLimit = 0;
  ...
  */
  ...
  /*
  fixIsOnSector search for the information if a device exist and if a battery is there
  */

  fixIsOnSector(){
    if(this.isOnSector == undefined){
      this.zipabox.isOn
    }
  }// end fixIsOnSector function
    ...
    return new Promise(function (resolve,reject){
      ...
      /* If not e check if it's allow to search one */
        ...
        .then(function setIfIsOnSector(deviceUUID){
          ...
          .then(function checkValueSector(){
            if(typeof(isOnSector) == "boolean")
              this.isOnSector = isOnSector;
            this.debug && this.log("[ZIPAC] [manageDeviceUUID] Is device on sector ?",isOnSector, this.debugContext);
            resolve(deviceUUID);
          }.bind(this));
        }.bind(this));
