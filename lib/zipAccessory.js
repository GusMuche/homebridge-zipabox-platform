...
class ZipAccessory{
  constructor(debug,log,zipabox,zAccessory,zService,zCharacteristic){
    ...
    this.centralDeviceStatus = false;
  ...
  */
  rechargeAccessoryFromCache(accessory){
    ...
    this.centralDeviceStatus = accessory.context.centralDeviceStatus;
  ...
  */
  fixContextFromConfig(config){
    ...
    this.platformAccessory.context.centralDeviceStatus = this.centralDeviceStatus;
  }
  ...
  */
  statusPolling(){
    ...
    /* Bloc to stop polling for central refresh */
    if(this.centralRefresh == true){
      this.debug && this.log("[ZIPAC] [statusPolling] Stopping refresh for central refresh. Reset parameter will be ignore.", this.debugContext);
  ...
  */
  getCentralDeviceStatus(){
    return this.centralDeviceStatus;
  }
  ...
  */
  setCentralDeviceStatus(status){
    if(status != true && status != false){
      ...
      this.centralDeviceStatus = status;
    }
  }
    ...
    return new Promise(function(resolve, reject){
      ...
      || error.message == "No JSON" || error.type == 'invalid-json'){ // || error.message == "Bad Request " > for test
        ...
        if(error.message == "No JSON" || error.type == 'invalid-json')
          this.log.warn("[ZIPAC] [reconnectAfterError] New implementation to reconnect V3", this.debugContext);
  ...
  */
  getDeviceStatus(){
    if(this.centralRefresh){
      return new Promise(function(resolve,reject){
        resolve(this.centralDeviceStatus);
      }.bind(this));
    }else{
      return this.zipabox.getDeviceStatus(this.deviceUUID,this.noStatus);
    }
