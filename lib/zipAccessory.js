...
class ZipAccessory{
  ...
  createService(){
    ...
    switch(this.service){
      case "switch":
        ...
        return new Service.Switch(this.name);//(this.name);
        break;
      case "light":
  ...
  */
  bindCharacteristic(){
    ...
    /* Switch to the correct service */
    if(this.platformAccessory.getService(Service.Switch)){// || accessory.getService(Service.Switch) || type == "outlet"){
  ...
  */
  ...
  /*
  getId is a shortcut to have the context.uuid
  */
  getId(){
    return this.platformAccessory.context.uuid;
  }
  ...
  getOnCharacteristicHandler(callback){
    ...
    .then(function (accessoryValue){
      ...
      if(typeof(returnedValue) != "boolean"){ // Check if returnedValue is a Boolean
        var error = new Error("Coding error in getOnCharacteristicHandler: returnedValue is not a boolean");
        this.log(error);
        throw error;
      }
