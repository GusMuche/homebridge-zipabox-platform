...
class ZipAccessory{
  ...
  */
  fixContextFromConfig(config){
    ...
    if(this.useEve != true && this.useEve != false){
      this.log.warn("Configuration error. Parameter useEve must be true or false. Forced to false");
      this.useEve = false;
    }
  ...
  */
  getStatusBattery(callback){
    ...
    if(this.deviceUUID == undefined){
      this.log.error("Bad request. device UUID is undefined", this.debugContext);
      callback(null,0);
      return;
    }
    ...
    .then(function (batteryValue){
      var underLevel = 0;
      this.debug && this.log("[ZIPAC] [getStatusBattery] Battery value :",batteryValue, this.debugContext);
    ...
    .catch(function manageError(error){
      if(error.message == "Not Found"){
        this.log.warn("[ZIPAC] [getStatusBattery] The battery level was not found. Please check config for noStatus = true. Plugin will force on sector.", this.debugContext);
        this.isOnSector = true;
        callback(null,0);
      }else{
        //this.log("Test Value in manage Error : ",deviceStatus);
        this.log.error("[ZIPAC] [getStatusBattery] Error not manage :",this.debugContext,error);
  ...
  */
  getOnB (callback) {
    ...
    if(this.type == "outlet"){
      if(this.uuidb == null){
        this.log("[ZIPAC] [getOnB] No uuidb configured. Return last value",this.lastValue, this.debugContext);
        callback(null, this.lastValue);
        return;
      }
    }
    ...
    .then(function (accessoryValue){
       ...
       if(this.type == "outlet"){
         // TO COMPLETE, test and check
         this.log("[ZIPAC] [getOnB] Outlet to manage in getOnB. returnedValue :",returnedValue, this.debugContext);
         this.log("[ZIPAC] [getOnB] Type of returnedValue :",typeof(returnedValue), this.debugContext);
