...
class ZipAccessory{
  constructor(debug,log,zipabox,zAccessory,zService,zCharacteristic){
    ...
    this.CustomGS = function() {
  		// Characteristic.call(this, 'CustomGS', 'E863F10D-079E-48FF-8F27-9C2605A29F52'); // Voltage
  		Characteristic.call(this, 'CustomGS', '00000068-0000-1000-8000-0026BB765291'); // battery percentage
  		this.setProps({
  	...
  	inherits(this.CustomGS, Characteristic);
    ...
    switch(this.type){
      case "switch":
        ...
        if(this.uuid=="disconnectBox" || this.uuid=="rebootBox" || this.uuid=="rebootHomebridge"){
          ...
          this.debug && this.log("[ZIPAC] [bindCharacteristic] Add custom Characteristic", this.CustomGS);
          if(fromCache){
            // this.platformAccessory.getService(Service.Switch)
            //   .getCharacteristic(this.CustomGS)
            //   .on('get', function(callback){callback(null,69);});
          }else{
            this.platformAccessory.getService(Service.Switch)
              .addOptionalCharacteristic(this.CustomGS);
            this.platformAccessory.getService(Service.Switch)
              .getCharacteristic(this.CustomGS)
              .on('get', function(callback){callback(null,69);});
          }
  ...
  */
  ...
  /* DEFINING OUTER FUNCTION AND PARAMETERS */
