...
class ZipAccessory{
  ...
  */
  createAccessory(config,uuidGen){
    ...
    if(this.debugPlatform || this.debug){
      this.debug = true;
      this.log.warn(">> Debug mode set for",this.name);
    }
  ...
  */
  rechargeAccessoryFromCache(accessory){
    ...
    this.specialType = accessory.context.specialType;
  ...
  */
  fixContextFromConfig(config){
    ...
    if(this.specialType != null){
      this.debug && this.log.warn("Special type ask by the user.")
      this.noStatus = true;
      this.batteryLimit = 0;
    }
    ...
    this.platformAccessory.context.specialType = this.specialType;
    ...
    return new Promise(function (resolve,reject){
      ...
      /* If not e check if it's allow to search one */
        ...
        .then(function giveDeviceUUID(deviceUUID){
          this.debug && this.log("Device UUID found in the box :",deviceUUID);
  ...
  */
  getOnCharacteristicCopyLast (callback){
    /* DEBUG */
    this.debug && this.log(">> Method getOnCharacteristicCopyLast");

    this.delay(2000,this.lastValue)
    .then(function sendTheCallback() { // here value is the lastValue
      var error = null;
      if(this.lastValue == undefined){
        error = "getOnCharacteristicCopyLast request without a setted value. Retry later."
  ...
  */
  getSpecial (callback){
    ...
    this.debug && this.log('>> Method getSpecial');
  ...
  */
  setSpecial (value, callback){
    /* Log to the console the value whenever this function is called */
    this.debug && this.log('>> Method setSpecial');

    /* Special type disconnectBox */
    if(this.specialType == "disconnectBox"){
      if(value){
        this.log.warn("User ask to disconnect the box")
        this.zipabox.logoutUser()
        .then(function giveAnswerOfLogout(response){
          if(response)
            this.log.warn("User is log out")
          this.updatePolling(); // Will set the button to false
          //this.platformAccessory.getService(Service.Switch).getCharacteristic(Characteristic.On).getValue();
          callback(null);
        }.bind(this))
        .catch(function manageUnknowError(error){
          this.log.error("Unknown error on setSpecial() :",error);
          callback(error);
      ...
      if (error.message == "Unauthorized" || error.message == "Unauthorized "){ // || error.message == "Bad Request " > for test
        ...
        .then(function manageAfterConnection(connectionLaunched){
          if(connectionLaunched == false){
            this.debug && this.log("Waiting for reconnection for device ", this.name);
        ...
        .then(function checkStatus(){
          ...
          !this.debug && this.log("The plugin has reconnect successfully to the box");
          this.updatePolling();
