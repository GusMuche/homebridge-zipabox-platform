    ...
    for(var i in this.uuids){
      if(this.uuids[i] == uuid){
        ...
        if(this.lastValues[i] == value){
          this.debug && this.log("[ZIPAC] [setLast] Found that new value is same as previous.", value, uuid, this.debugContext);
        }else{
          this.debug && this.log.warn("[ZIPAC] [setLast] Found that value as change. Update to homebridge.", value, uuid, this.debugContext);
