...
*/
class ZipAccessory{
  constructor(debug,log,zipabox,zAccessory,zService,zCharacteristic,accessoryJSON,uuidGen,timePolling){
  ...
  */
  bindCharacteristic(type,fromCache = false){
    ...
    if(this.timePolling > 0 && !this.centralRefresh)
      this.statusPolling();
  ...
  */
  identifyZipAccessory(){
    ...
    this.log.warn(this.debugContext,"Less request ? :",this.centralRefresh);
  ...
  */
  ...
  /*
  getZipaUUID is a shortcut to have the uuid of the attribute
  */
  getZipaUUID(){
    return this.uuid;
  ...
  */
  getOn(callback){
    ...
    /* Return error if device is not online in Big Refresh status */
    if(this.centralRefresh && this.deviceStatus == false){
  ...
  */
  setLast(value){
    /* DEBUG */
    this.debug && this.log("[ZIPAC] > Method setLast :", value, this.debugContext);

    this.lastValue = value;
    // update the central Refresh boolean
    if(this.type != "alarm")
      this.centralRefresh = true;
  }
