...
class ZipAccessory{
    ...
    for (i in services){
      ...
      switch(this.types[i]){
        ...
        case "co2":
          ...
          service = new Service.CarbonMonoxideSensor(name);
          break;
        case "battery":
  ...
  bindAllCharacteristics(){
    ...
    for (i in this.services){
      ...
      if(this.batteryLimit > 0){
        this.bindBatteryLowStatus(i,types[i]);
      }
    }
  ...
  */
  directUpdate(index,value){
    ...
    switch(type){
      ...
      case "dimmer":
        service.updateCharacteristic(Characteristic.On, value);
        service.updateCharacteristic(Characteristic.Brightness).getValue();
        break;
      case "outlet":
        service.updateCharacteristic(Characteristic.On, value);
  ...
  */
  setLast(value,uuid){
    ...
    for(i in this.uuids){
      if(this.uuids[i] == uuid){
        find = true;
        if(this.lastValue[i] == value){
          this.debug && this.log.error("[ZIPAC] [setLast] Found that new value is same as previous.", value, uuid, this.debugContext);
        }else{
          this.debug && this.log.error("[ZIPAC] [setLast] Found that value as change. Update on the box.", value, uuid, this.debugContext);
          this.updateValueAPI(value,i);
          .then(function updateCharacteristic(){
  ...
  /*
  updateValueAPI send the new value of the attribute to the API
  */
  updateValueAPI(value,index){
    /* DEBUG */
    this.debug && this.log("[ZIPAC] > Method updateValueAPI", value, index, this.debugContext);

    /* Send the request */
    this.zipabox.putAttributesValue(this.uuids[index],value)
    ...
    .catch(function manageError(error) {
      this.log.error("[ZIPAC] [updateValueAPI] Error :", error, this.debugContext);
    });
  }
