...
class ZipAccessory{
  ...
  /*
  desactivate is used to set the sttus of the instance to "delete"
  ...
  */
  fixContextFromConfig(config){
    ...
    if(this.useCache && this.timePolling < 100){
      this.log.warn("Using cache with no refresh will not refresh the value of the accessory. Using Cache is forced to false.")
      this.useCache = false;
    }
  ...
  getOn(callback){
    ...
    if(this.useCache){
      if(this.getTimer == undefined){ // First launch > initiate the timer
        ...
        if(timeDown >= this.timePolling / (1000)){
          needReconnection = true;
    ...
    /* Get level based on cache or request the value */
    if(needReconnection == false && this.useCache == true){ // useCache without reconnection needed
      this.debug && this.log("[ZIPAC] [getOn] Using cache. Return cached value", this.lastValue, this.debugContext);
      this.getOnOngoing = false;
      callback(error, this.lastValue);
    }else{ // no cache use or reconnection needed
      this.getOnAPI()
