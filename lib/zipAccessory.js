...
class ZipAccessory{
  constructor(debug,log,zipabox,zAccessory,zService,zCharacteristic,accessoryJSON,uuidGen){
    ...
    this.alarmPutTimer = undefined;
        // this.timePolling = timePolling; // only for alarm (maybe)
        // this.useCache = false;
    ...
    this.batteryLevel = 100;
        ...
        // this.centralRefresh = centralRefresh;
    ...
    this.firstType = this.config.accessories[0].type || undefined;
    if(this.firstType == undefined)
      this.log.error("[ZIPAC] Bad type inserted for the accessory", this.name, "Check config.json");
    ...
    /* Create all services */
    this.addAllServices(this.config.services);
  ...
  configAccessory(config){
    ...
    if(this.hidden != true && this.hidden != false){
      ...
      this.hidden = false;
        ...
        // this.platformAccessory.context.centralRefresh = this.centralRefresh;
        // this.platformAccessory.context.centralDeviceStatus = this.deviceStatus;
  }
      ...
      //   this.centralRefresh = accessory.context.centralRefresh;
      //   this.deviceStatus = accessory.context.centralDeviceStatus;
  ...
  addServices(services){
    ...
    for (i in services){
      ...
      if(this.testValues[i] != null)
        this.debug && this.log("[ZIPAC] [fixContextFromConfig] Test value fixed by user at ", this.testValue, this.debugContextServices[i]);
      ...
      switch(this.types[i]){
        ...
        case "alarm":
          ...
          NO_FAULT = 0;
          GENERAL_FAULT = 1;*/
          this.statusFault[i] = 0;
          break;
        default:
          this.log.warn("[ZIPAC] [addServices] Add a default Switch service - Check your configuration.", this.debugContextServices[i]);
          serviceToReturn = new Service.Switch(name);
      } // end switch types
      if(this.platformAccessory.getService(serviceToReturn)){
        this.log("[ZIPAC] [addServices] Service already exist for this accessory", this.debugContextServices);
      }else{
        this.log("[ZIPAC] [addServices] Service didnt exist for this accessory", this.debugContextServices);
      }
      this.servicesNames[i] = this.name + this.uuids[i];
      this.platformAccessory.addService(serviceToReturn,this.servicesNames[i],this.uuids[i]);
      this.services[i] = serviceToReturn;
      this.nbServices++;
    } // end for services[i]
  } // end createService()
  ...
  */
  bindCharacteristic(index,type,uuid,serviceName){
    /* DEBUG */
    this.debug && this.log("[ZIPAC] > Method bindCharacteristic", this.debugContextServices[i]);
    ...
    switch(type){
      case "switch":
        ...
        if(uuid=="disconnectBox" || uuid=="rebootBox" || uuid=="rebootHomebridge"){
          this.platformAccessory.getService(serviceName)
            ...
            .on('get', this.getSpecial(uuid).bind(this))
            .on('set', this.setSpecial(uuid).bind(this));
        }else{
          /* And a normal switch */
          this.platformAccessory.getService(serviceName)
            .getCharacteristic(Characteristic.On)
            .on('get', function(callback){
              this.getCharacteristicValue(index, callback);
            }.bind(this))
            .on('set', function(value, callback){
              this.setOn(index, value, callback);
            }.bind(this));
        }
        break;
      case "light": // TO TEST AFTER SWITCH
        this.platformAccessory.getService(serviceName)
          .getCharacteristic(Characteristic.On)
      ...
      case "dimmer":
        this.platformAccessory.getService(serviceName)
          .getCharacteristic(Characteristic.On)
          .on('get', this.getOnRange(uuid).bind(this))
          .on('set', this.setOnRange(uuid).bind(this));
        this.platformAccessory.getService(serviceName)
  ...
  */
  bindBatteryLowStatus(){
    ...
    if(this.useEve){
      ...
      //   this.debug && this.log("[ZIPAC] [statusPolling] Central refresh ?", this.centralRefresh);
      //
  ...
  /*
  updateAllServices loop through all services and update each
  */
  allRefreshGet(){
  ...
  /*
  update launch the get method to refresh the value of the accessory
  depend of his type
  */
  refreshGet(index){
    /* DEBUG */
    this.debug && this.log("[ZIPAC] > Method refreshGet", index, this.debugContext);
    ...
    if(index < 0 || index >= this.nbServices){
      this.log.error("[ZIPAC] [refreshGet] Bad index request :", index, this.debugContext);
      return;
    }
    /* get the information */
    ...
    switch(type){
      ...
      case "alarm":
        ...
        break;
      default:
        this.log.error("[ZIPAC] [refreshGet] No type found for", type);
    } // end switch
  ...
  */
  ...
  /*
  getZipaUUID is a shortcut to have the uuid of the attributes
  */
  getZipaUUIDs(){
    return this.uuids;
  } // end function getZipaUUID
  ...
  */
  getCharacteristicValue(index, callback){
    ...
    var error = null;
    if(this.attributesValues[index]==null || this.attributesValues[index]==undefined || index > this.nbServices ){
    ...
    callback(error, this.attributesValues[index]);
  }
  ...
  //       //       this.getTimer = now;
  //       //     }else{
  //       //       this.debug && this.log("[ZIPAC] [getOn] No refresh needed. Use cached value timeDown :",timeDown,this.debugContext);
  //       //     }
  //       //   }
  //       // } // End if useCache
  //
  //   this.debug && this.log("[ZIPAC] [getOn] Central Refresh ?",this.centralRefresh,this.debugContext);
  //
  //   /* Return error if device is not online */
  //   if(this.deviceStatus == false){
  //     callback("Device not online",false);
  //   }
  //
  //   /* Get level based on cache or request the value */
  //   // Firstly use Cache for quick request or no use of API
  //   if(this.getOnOngoing || (!needApiResquest && this.useCache) || this.centralRefresh){ // useCache without reconnection needed
  //     this.debug && this.log("[ZIPAC] [getOn] Using cache. Return cached value :", typeof(this.lastValue), this.lastValue, this.debugContext);
  //     this.getOnOngoing = false;
  //     callback(null, this.lastValue);
  ...
  //       this.getOnOngoing = false;
  //       this.lastValue = returnOfApi;
  //       this.debug && this.log("[ZIPAC] [getOn] Callback the API request value",this.lastValue, this.debugContext);
  //       callback(null, this.lastValue);
  //     }.bind(this))
  //     .catch(function manageError(error){
  ...
  //     this.lastValue = this.translateAttribute(accessoryValue);
  ...
  */
  ...
  /*
  setLast is used to fix the lastValue as the cache value (central refresh)
  Why used only with central Refresh, this put the boolean to true
  */
  setLast(value,uuid){
    /* DEBUG */
