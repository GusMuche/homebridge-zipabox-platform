  ...
  statusPolling(){
    ...
    .catch(function manageError(error){
      ...
      if(this.connectionOnGoing){
        ...
        return this.reconnectAfterError();
      }
    }.bind(this));
    ...
    return new Promise(function(resolve, reject){
      ...
      || error.message == "No JSON" || error.type == 'invalid-json'){
        ...
        .then(function reportStatus(){
          this.debug && this.log("[PTFM] [reconnectAfterError] Reconnection success > get Device Status");
          !this.debug && this.log("[PTFM] [reconnectAfterError] The plugin has reconnect successfully to the box");
        }.bind(this))
        .catch(function manageError(error){
           reject(error);
