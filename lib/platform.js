...
class ZipaPlatform{
  ...
  */
  addAccessory(accessoryJSON){
    ...
    this.debug && this.log("[PTFM] [addAccessory] Accessory first UUID :",accessoryJSON.accessories[0].uuid);
    ...
    var uuidGen = UUIDGen.generate(accessoryJSON.accessories[0].uuid+accessoryJSON.name);
  ...
  */
  statusPolling(){
    ...
    .then(function updateAccessories(){
      ...
      for (var zipAccesory of this.zipAccessories){
        zipAccesory.updatePolling(); // update value
      }
    }.bind(this))
    .catch(function manageError(error){
