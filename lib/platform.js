...
class ZipaPlatform{
  ...
  */
  statusPolling(){
    ...
    .then(function savePartitionsStatus(newSecurityStatuses){ // save the collected statuses inside this.alarmStates
      if(this.alarmIsConfigured){
        var index = 0;
        for(var alarmUUID of this.partitions){
          this.debug && this.log("[PTFM] [statusPolling] Save new status for alamrUUID - status - index",alarmUUID, newSecurityStatuses[index], index);
          this.alarmStates[index] = newSecurityStatuses[index];
          index++;
        }
      }
    }.bind(this))
    ...
    .then(function updateAttributesValues(newAttributesValues){
      ...
      for (var zipAccesory of this.zipAccessories){
        ...
        /* Second manage alarm */
          ...
          this.debug && this.log("[PTFM] [statusPolling] Device ",zipAccesory.getDeviceUUIDfix(), "is an alarm, update Status ",zipAccesory.getDebugContext());
          var partitionIndex = this.getAlarmIndex(zipAccesory.getUUID());
          if(partitionIndex >= 0)
            zipAccesory.setAlarm(this.alarmStates[partitionIndex]);
  ...
  */
  ...
  /*
  get Alarm index return the index of the partitions in the arrays base on the partition UUID
  */
  getAlarmIndex(partitionUUID){
    /* DEBUG */
    this.debug && this.log("[PTFM] > Method getAlarmIndex");
    var foundedIndex = -1;
    for(var index in this.partitions){
      if(partitionUUID == this.partitions[index])
        foundedIndex = index;
    } // end for for this.partitions
    /* Check if found or not */
    if(foundedIndex == -1)
      this.log.error("[PTFM] [getAlarmIndex] bad index found (-1). Status will be wrong.");
    return foundedIndex;
  }// end of getAlarmIndex
