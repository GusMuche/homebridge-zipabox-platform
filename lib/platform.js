  ...
  */
  checkUUIDAttributes(){
    ...
    .then(function checkUUIDs(attributesUUID){
      ...
      var zipAccessoriesOld = this.zipAccessories;
      this.debug && this.log("[PTFM] [checkUUIDAttributes] Length of zipAccessories already stored :",zipAccessoriesOld.length);
      this.debug && this.log("[PTFM] [checkUUIDAttributes] Length of attributesUUID from the box :",attributesUUID.length);
      for(var indexSearch = 0; indexSearch < zipAccessoriesOld.length; indexSearch++){
        this.debug && this.log("[PTFM] [checkUUIDAttributes] Actual index :",indexSearch);
        var isValable = false;
        if(zipAccessoriesOld[indexSearch].uuid == "rebootBox" || zipAccessoriesOld[indexSearch].uuid == "rebootHomebridge" || zipAccessoriesOld[indexSearch].uuid == "disconnectBox"){
          isValable = true;
        }else{
          for(var indexCheck = 0; indexCheck < attributesUUID.length; indexCheck++){
            if(zipAccessoriesOld[indexSearch].uuid == attributesUUID[indexCheck].uuid)
              isValable = true;
          } // end for
        } // end else
        ...
        /* Manage error */
        if(!isValable){
          this.log.error("[PTFM] [checkUUIDAttributes] The UUID of the accessory",zipAccessoriesOld[indexSearch].name, this.accessoriesConfig[indexSearch].uuid, "is not available in the box. Please check config. Accessory will not be added");
          platformAccessoryToDelete.push(zipAccessoriesOld[indexSearch].getPlatformAccessory());
        }else{
          this.debug && this.log("[PTFM] [checkUUIDAttributes] UUID of accessory state is in the box. That's good.",zipAccessoriesOld[indexSearch].uuid);
        }
      } // end for
