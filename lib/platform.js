...
class ZipaPlatform{
  ...
  */
  constructor (log, config, homeBridgeApi) {
    ...
    /* Zipabox API Object */
    this.zipabox = new ZipaboxApi(this.debug,this.baseURL,this.log,this.user,this.password);
    //this.zipAccessory = new ZipAccessory(this.debug,this.log,this.zipabox,Accessory,Service,Characteristic);
  ...
  /*
    Method connectTheBox. Will simply... connect the box = init > login
  */
  connectTheBox(){
    ...
    .catch(function manageError(error) {
      this.log.error("Error on connectTheBox : ",error);
  ...
  */
  configureAccessory(accessory){
    ...
    // if(this.reset){
    //   this.debug && this.log("User request to delete the cached accessory. Here ", accessory.displayName)
    //   // this.accessories = [];
    ...
    var alreadyStored = this.isAccessoryStored(accessory.context.uuid);
    ...
    for (var index in this.accessoriesConfig){
      if(accessory.context.uuid === this.accessoriesConfig[index].uuid)
        isConfigured = true;
    }
    ...
    if(!alreadyStored && isConfigured){
      ...
      zipAccessory = new ZipAccessory(this.debug,this.log,this.zipabox,Accessory,Service,Characteristic);
  ...
  */
  didFinishLaunching(){
    ...
    for ( var index in this.accessoriesConfig){
      this.debug && this.log("In didFinishLaunching > add the accessory :",this.accessoriesConfig[index].name);
      this.addAccessory(this.accessoriesConfig[index]);
    }
    /* Update the cache after launch all the accessories */ //TODO : check if strictly needed
    this.homebridge.updatePlatformAccessories(this.acceessories);
  ...
  */
  addAccessory(accessoryJSON){
    ...
    var alreadyExist = this.isAccessoryStored(accessoryJSON.UUID);
    this.debug && this.log("Accessory to add already exist ? :",alreadyExist);
    if(alreadyExist){
      /* Accessory is Stored, need to set it reachable */
      this.zipAccessories[this.getZipAccessoryIndexByUUID(accessoryJSON.UUID)].updateReachability(true);
    }else{
      ...
      zipAccessory.updateReachability(true); // TODO test without
      this.zipAccessories.push(zipAccessory);
      this.debug && this.log("Length of zipAccessories :",this.zipAccessories.length);
  ...
  /*
  configurationRequestHandler not implemented
  */
  ...
  */
  removeAccessory(accessory){
    ...
    if(this.isAccessoryStored(accessory.context.uuid))
      delete this.zipAccessories[this.getZipAccessoryIndexByUUID(accessory.context.uuid)];
    this.homebridge.updatePlatformAccessories([accessory]); //FIXME : do we need ?
  }
  ...
  */
  removeAllAccessories(){
    /* DEBUG */
    this.debug && this.log("> Method removeAllAccessories");
    var allPlatformAccessories = [];
    for (zipAccesory in this.zipAccessories){
      allPlatformAccessories.push(zipAccessory.platformAccessory);
    }
  ...
  */
  isAccessoryStored(accessoryConfigUUID){
    /* context is like the config json */
    if (this.getZipAccessoryIndexByUUID(accessoryConfigUUID) != -1)
      return true;
    else
      return false;
  ...
  /*
  getZipAccessoryIndexByUUID return the index (int) for a specific uuid (not the generate one)
  */
  getZipAccessoryIndexByUUID(uuid){
      /* DEBUG */
      this.debug && this.log("> Method getZipAccessoryIndexByUUID");
      for (var index = 0; index < this.zipAccessories.length; index++) {
        if(this.zipAccessories[index].getId() == uuid){
          this.debug && this.log("Index found :",index);
          return index;
        }
      } // end for
      this.log.warn("Accessory requested not found for uuid",uuid);
      return -1;
  ...
  */
  manageDevicesUUID(){
    /* DEBUG */
    this.debug && this.log("> Method manageDevicesUUID");
    var promiseArray = [];
    this.debug && this.log("Length of zipAccessories :",this.zipAccessories.length);
    for (var index = 0; index < this.zipAccessories.length; index++) {
      var zipAccessory = this.zipAccessories[index];
      // this.debug && this.log("ZipAccesory :",zipAccessory);
      // this.debug && this.log("Type of zipAccesory :",typeof(zipAccessory));
      promiseArray.push(zipAccessory.getDeviceUUID());
    } // end for
    // for (var zipAccessory in this.zipAccessories){
    //   this.debug && this.log("ZipAccesory :",zipAccesory);
    //   this.debug && this.log("Type of zipAccesory :",typeof(zipAccesory));
    //   promiseArray.push(zipAccessory.getDeviceUUID());
    // }
