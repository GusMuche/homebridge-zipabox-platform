...
class ZipaPlatform{
  ...
  */
  constructor (log, config, homeBridgeApi) {
    ...
    if(this.boxSerial == "ERROR" && this.IP == "remoteV3"){
      this.log.error("--- !!! --- ");
    ...
    this.alarmIsConfigured = false; // will be change to true if user specify an alarm
        //
        // /* Big Refresh option */
        // this.centralRefresh = config["lessRequest"] || false;
        // if(this.centralRefresh != true && this.centralRefresh != false){
        //   this.log.warn("[PTFM] WARNING : config error : centralRefresh is not set to true or false. Forced to false");
        //   this.centralRefresh = false;
        // }
        // if(this.centralRefresh)
        //   this.log.warn("[PTFM] lessRequest option activated - ignore accessory refresh to limit API update");
  ...
  */
  configureAccessory(accessory){
    ...
    /* No more reuse from archived accessories, direct delete */
    this.cachedAccessoriesToDelete.push(accessory);
        ...
        //
        // /* Check if accessory is hidden or not */
        // var isHidden = accessory.hidden;
        //
        // /* Manage the system with the two status of the accessory */
        // if(!alreadyStored && isConfigured && !isHidden){
        ...
        //   this.debug && this.log("[PTFM] [configureAccessory] Accessory not stored but configured and not hidden > store.");
        // }else if (!isConfigured || isHidden) {
        //   this.cachedAccessoriesToDelete.push(accessory);
        //   // this.removePlatformAccessory(accessory);
        //   this.debug && this.log("[PTFM] [configureAccessory] Accessory not stored and not configured or hidden > saved for delete.");
        // }else{
  ...
  */
  addAccessory(accessoryJSON){
    ...
    this.debug && this.log("[PTFM] [addAccessory] Accessory UUIDGen :",uuidGen);
      ...
      var zipAccessory = new ZipAccessory(this.debug,this.log,this.zipabox,Accessory,Service,Characteristic,accessoryJSON,uuidGen,this.timePolling);
  ...
  connectTheBox(){
    ...
    .then(function connectSecurityIfNeeded(notUsedReturn){
      if(this.alarmIsConfigured && !this.connectionError){
        ...
        //     if(this.centralRefresh){
        //       this.devicesStatuses[zipAccesory.getDeviceUUID] = undefined;
        //       this.attributesValues[zipAccesory.getUUID] = undefined;
        //     }else{
    ...
    .then(function endInformationAfterAPIRequest(notUsedReturn){
      ...
      if(this.alarmIsConfigured && !this.connectionError)
        this.debug && this.log("[PTFM] [connectTheBox] User is connected to the security partition");
          // /* Launch polling */ > see didFinishLaunching
          // this.timeOut = setTimeout(function refreshAccessories(){
          //   this.statusPolling();
          // }.bind(this),this.timePolling)// end function of timeout
      this.connectionOnGoing = false;
  ...
  */
  statusPolling(){
    ...
    .then(function updateDevicesStatus(newDevicesStatuses){
      ...
      for (var zipAccesory of this.zipAccessories){
        ...
        if(status != undefined){
          ...
          zipAccesory.setCentralDeviceStatus(status);
    ...
    .then(this.zipabox.getAllAttributesValues.bind(this))
    .then(function updateAttributesValues(newAttributesValues){
      var value = null;
      for (var zipAccesory of this.zipAccessories){
        value = this.getNewAttributeValue(newAttributesValues,zipAccesory.getZipaUUID());
        if(value != undefined){
          value = zipAccesory.translateAttribute(value)
          this.attributesValues[zipAccesory.getZipaUUID()] = value;
          zipAccesory.setLast(value);
          this.debug && this.log("[PTFM] [statusPolling] Attributes ",zipAccesory.getZipaUUID(), "value", value, zipAccesory.getDebugContext());
        }else{
          this.log.error("[PTFM] [statusPolling] No attributes found for UUID",zipAccesory.getZipaUUID(),zipAccesory.getDebugContext());
        }
      }
    }.bind(this))
    .then(function updateAccessories(){
      this.debug && this.log("[PTFM] [statusPolling] Update all the accessories to HomeKit");
      for (var zipAccesory of this.zipAccessories){
        zipAccesory.updatePolling();
      }
    }.bind(this))
