...
class ZipaPlatform{
  ...
  */
  connectTheBox(){
    ...
    .then(this.loadAllPartition.bind(this))
  ...
  */
  statusPolling(){
    ...
    .then(function refreshAlarmPartitions(){
      ...
      }else{ // !alarmIsConfigured
        resolve(true);
      }
    }.bind(this))
    .then(function setPartitionsStatus(responseArray){
      if(this.alarmIsConfigured){
    ...
    .then(function updateAttributesValues(newAttributesValues){
      ...
      for (var zipAccesory of this.zipAccessories){
        ...
        if(zipAccesory.isSpecial()){
          // do nothing ?
        }else if(zipAccesory.hasAlarm() && this.alarmIsConfigured){
    ...
    .catch(function manageError(error){
      this.debug && this.log.error("[PTFM] [statusPolling] Error in update search :",error,error.message);
  ...
  */
  ...
  /*
  If alarm is configured, the method load all partitions UUID from the config and
  keep only the validate one
  return a promise, null if no partitions configured
  */
  loadAllPartition(){
    /* DEBUG */
    this.debug && this.log("[PTFM] > Method loadAllPartition");

    if(!this.alarmIsConfigured){
      this.debug && this.log.warn("[PTFM] [loadAllPartition] No alarm to load");
      return null;
    }else{
      this.debug && this.log.warn("[PTFM] [loadAllPartition] Launch the alarm partition");
      ...
      .then(function loadJson(partitionsJson){
        ...
        for(var indexConfig in this.partitions){
          ...
          if(theConfigIsOk){
            this.debug && this.log("[PTFM] [loadAllPartition] The partition UUID is found in the list",this.partitions[indexConfig]);
            validatedPartitions.push(this.partitions[indexConfig]);
          }else{
            this.log.error("[PTFM] [loadAllPartition] The partition UUID is not found in the box. Update will be stopped",this.partitions[indexConfig]);
          }
        }
        this.partitions = validatedPartitions;
        resolve(validatedPartitions);
      }.bind(this));
    }
  }
  ...
  reconnectAfterError(error){
    ...
    return new Promise(function(resolve, reject){
      ...
      || error.message == "No JSON" || error.type == 'invalid-json'){
        ...
        //     this.debug && this.log("[PTFM] [reconnectAfterError] Waiting for reconnection for device ", this.name);
