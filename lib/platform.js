  ...
  */
  removePlatformAccessory(accessoryPlatform){
    ...
    if(this.isAccessoryStored(accessoryPlatform.UUID)){
      ...
      this.debug && this.log("[PTFM] [removePlatformAccessory] Accessory to delete found at index :",this.getZipAccessoryIndexByUUID(accessoryPlatform.UUID));
      delete this.zipAccessories[this.getZipAccessoryIndexByUUID(accessoryPlatform.UUID)];
    }else{
  ...
  */
  checkUUIDAttributes(){
    ...
    .then(function checkUUIDs(attributesUUID){
      for(var indexSearch in this.zipAccessories){
        var isValable = false;
        if(this.zipAccessories[indexSearch].uuid == "rebootBox" || this.zipAccessories[indexSearch].uuid == "rebootHomebridge" || this.zipAccessories[indexSearch].uuid == "disconnectBox"){
          isValable = true;
        }else{
          for(var indexCheck in attributesUUID){
        ...
        /* Manage error */
        if(!isValable){
          this.log.error("[PTFM] [checkUUIDAttributes] The UUID of the accessory",this.accessoriesConfig[indexSearch].name, this.accessoriesConfig[indexSearch].uuid, "is not available in the box. Please check config. Accessory will be not added");
          this.removePlatformAccessory(this.zipAccessories[indexSearch].getPlatformAccessory());
        }else{
          this.debug && this.log("[PTFM] [checkUUIDAttributes] UUID of accessory state is in the box. That's good.",this.accessoriesConfig[indexSearch].uuid);
        }
      } // end for
    }.bind(this))
