...
class ZipaPlatform{
  ...
  */
  constructor (log, config, homeBridgeApi) {
    ...
    this.debug = false;
    if (!config) {
      this.log.warn(`Ignoring ZipaboxPlatform  setup because it is not configured`);
      this.debug= true; // TODO delete this if oll go coorectly
      this.disabled = true;
      return;
    }
    /* Debug */
    this.debug = config["debug"];
    this.debug && this.log.warn("DEBUG - Debug mode configured by user for ALL the accessory.");
    ...
    if(this.pin != "noPin")
      this.debug && this.log("Pin for alarm option is specified");
  ...
  */
  connectTheBox(){
    /* DEBUG */
    this.debug && this.log("> Method connectTheBox");
    this.zipabox.connectUser()
    // .then(this.manageDevicesUUID.bind(this))
    // .catch(function manageConnectionError(error){
    //   if(error.message == "reconnectionOnTheWay"){
    //     //setTimeout(this.log("Waited for reconnection."),7000);
    //     this.log.warn("Reconnection on the way for platform.")
    //   }else{
    ...
    .catch(function manageError(error) {
      this.log.error("Unexpexted error on connectTheBox before security : ",error);
    ...
    .then(function firstRefreshOfAccessories(notUsedReturn){
      ...
      if(this.alarmIsConfigured)
        this.log("User is connected to the security partition");
    ...
    .catch(function manageError(error) {
      this.log.error("Error on connectTheBox2 :",error);
      throw new Error(error);
    }.bind(this));
  ...
  */
  configureAccessory(accessory){
    ...
    if(this.disabled){
      this.log.warn("Found an orphelin Accessory configured before. Delete them :",accessory.displayName);
  ...
  */
  didFinishLaunching(){
    ...
    if(this.reset){
      this.debug && this.log("User request to delete the cached accessory.");
  ...
  */
  statusPolling(){
    /* DEBUG */
    this.debug && this.log("> Method statusPolling");
  ...
  */
  removePlatformAccessory(accessory){
    /* DEBUG */
    this.debug && this.log("> Method removePlatformAccessory");
    /* Retrait de l'accessoire de homebridge */
    accessory.updateReachability(false);
    this.homebridge.unregisterPlatformAccessories("homebridge-zipabox-platform", "ZipaboxPlatform", [accessory]);
    /* Retrait de la liste des accessoires */
    if(this.isAccessoryStored(accessory.UUID))
      delete this.zipAccessories[this.getZipAccessoryIndexByUUID(accessory.UUID)];
  ...
  */
  removeAllAccessories(){
    ...
    for (var index = 0; index < this.zipAccessories.length; index++) {
      this.debug && this.log("Remove",this.zipAccessories[index].platformAccessory.displayName);
  ...
  */
  isAccessoryStored(accessoryPlatformUUID){
    ...
    if (this.getZipAccessoryIndexByUUID(accessoryPlatformUUID) != -1)
      return true;
    else
      return false;
  ...
  */
  getZipAccessoryIndexByUUID(accessoryPlatformUUID){
      ...
      //this.log.warn("Accessory requested not found for uuid",accessoryPlatformUUID);
      return -1;
  ...
  */
  checkIfAlarm(){
    /* DEBUG */
    this.debug && this.log("> Method checkIfAlarm");
    for(var index in this.accessoriesConfig){
      if(this.accessoriesConfig[index].type == "alarm")
        this.alarmIsConfigured = true;
    }
    if(this.alarmIsConfigured && this.pin == "noPin")
      this.log.error("No pin configured with an alarm specified. Plugin will crash. See config.json");
  }
